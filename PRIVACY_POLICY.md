# Privacy Policy for KCS Meet

**Last Updated:** December 27, 2024  
**Effective Date:** December 27, 2024

## 1. Introduction

Krishna Consciousness Society ("we," "our," or "us") operates KCS Meet, a video conferencing platform designed for spiritual communities. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application and web services.

**By using KCS Meet, you agree to the collection and use of information in accordance with this policy.**

## 2. Information We Collect

### 2.1 Personal Information You Provide

We collect the following personal information when you create an account:

- **Name** (First name and Last name)
- **Email Address** (for account creation and authentication)
- **User ID** (automatically generated unique identifier)
- **Profile Picture** (optional, uploaded by you)
- **Username** (for display in meetings)

### 2.2 Information Collected Automatically

When you use KCS Meet, we automatically collect:

- **Device Information**: Device type, operating system, unique device identifiers
- **Usage Data**: Meeting participation history, feature usage, session duration
- **Log Data**: IP address, browser type, pages visited, time and date of visits
- **Location Data**: Approximate location (city/country level) based on IP address
- **Audio and Video Data**: During meetings (processed in real-time, not stored unless recording is enabled)

### 2.3 Payment Information

When you make payments through Super Chat:

- **Transaction Amount and Currency**
- **Payment Method Type** (via Cashfree payment gateway)
- **Transaction ID and Status**

**Note:** We do not store your credit card, debit card, or bank account details. All payment information is processed and secured by Cashfree, our PCI-DSS compliant payment processor.

### 2.4 Meeting Content

- **Chat Messages**: Text messages sent during meetings
- **Meeting Recordings**: Audio and video recordings (only when recording feature is activated by meeting host)
- **Poll Responses**: Your votes and responses to in-meeting polls
- **Super Chat Messages**: Paid messages and associated metadata

## 3. How We Use Your Information

We use collected information for the following purposes:

### 3.1 Primary Functions

- **Account Management**: Create and manage your user account
- **Authentication**: Verify your identity and secure your account
- **Meeting Services**: Enable you to join, host, and participate in video meetings
- **Communication**: Facilitate chat, audio, and video communication during meetings
- **Feature Delivery**: Provide features like virtual backgrounds, polls, and Super Chat

### 3.2 Analytics and Improvements

- **Service Improvement**: Analyze usage patterns to enhance user experience
- **Performance Monitoring**: Monitor and improve platform performance and reliability
- **Bug Fixes**: Identify and resolve technical issues
- **Feature Development**: Develop new features based on user needs

### 3.3 Legal and Security

- **Compliance**: Comply with legal obligations and respond to legal requests
- **Security**: Detect, prevent, and address fraud, security breaches, and prohibited activities
- **Terms Enforcement**: Enforce our Terms and Conditions

### 3.4 Communications

- **Service Updates**: Notify you about service changes, updates, and maintenance
- **Support**: Respond to your inquiries and provide customer support
- **Marketing**: Send promotional content (only with your consent, you may opt out anytime)

## 4. Data Sharing and Disclosure

### 4.1 We Share Your Information With:

**Third-Party Service Providers:**

- **Clerk**: Authentication and user management services
- **Stream.io**: Video and audio streaming infrastructure
- **Supabase**: Database and real-time data synchronization
- **Cashfree**: Payment processing for Super Chat donations
- **Vercel**: Hosting and content delivery
- **Firebase**: Push notifications and analytics

**Legal Requirements:**

We may disclose your information if required by law, court order, or government regulation, or if we believe disclosure is necessary to:
- Comply with legal obligations
- Protect our rights, property, or safety
- Prevent fraud or security issues
- Respond to emergency situations

**Business Transfers:**

In the event of a merger, acquisition, or sale of assets, your information may be transferred to the acquiring entity.

### 4.2 We Do NOT:

- Sell your personal information to third parties
- Share your information for third-party advertising
- Use your meeting content for advertising purposes
- Access your private meetings without authorization

## 5. Data Retention

### 5.1 Retention Periods

- **Account Data**: Retained while your account is active and for 90 days after account deletion
- **Meeting Recordings**: Stored as configured by meeting host, default 180 days
- **Chat Messages**: Retained for 90 days after meeting ends
- **Payment Records**: Retained for 7 years for tax and legal compliance
- **Log Data**: Retained for 90 days for security and debugging purposes

### 5.2 Deletion Rights

You have the right to request deletion of your personal data. See Section 9 for details.

## 6. Data Security

We implement industry-standard security measures to protect your information:

### 6.1 Technical Safeguards

- **Encryption in Transit**: All data transmitted between your device and our servers is encrypted using TLS 1.2 or higher
- **Encryption at Rest**: Sensitive data stored in databases is encrypted
- **Secure Authentication**: Password hashing using bcrypt, JWT token-based sessions
- **Access Controls**: Role-based access control (RBAC) for administrative functions
- **Regular Security Audits**: Periodic security assessments and vulnerability scanning

### 6.2 Physical Safeguards

- **Cloud Infrastructure**: Hosted on secure, SOC 2 compliant cloud providers
- **Data Centers**: Industry-leading data centers with physical security controls

### 6.3 Administrative Safeguards

- **Employee Training**: Regular security awareness training for staff
- **Limited Access**: Access to personal data restricted to authorized personnel only
- **Incident Response**: Documented procedures for responding to security incidents

**Note:** While we strive to protect your information, no method of transmission over the Internet or electronic storage is 100% secure. We cannot guarantee absolute security.

## 7. Your Privacy Rights

Depending on your location, you may have the following rights:

### 7.1 Access and Portability

- **Right to Access**: Request a copy of your personal data
- **Data Portability**: Receive your data in a structured, machine-readable format

### 7.2 Correction and Deletion

- **Right to Rectification**: Correct inaccurate or incomplete personal data
- **Right to Erasure**: Request deletion of your personal data (subject to legal retention requirements)

### 7.3 Control and Objection

- **Right to Restrict Processing**: Limit how we process your data
- **Right to Object**: Object to processing based on legitimate interests
- **Right to Withdraw Consent**: Withdraw consent for data processing (where consent is the legal basis)

### 7.4 How to Exercise Your Rights

To exercise any of these rights, contact us at:
- **Email**: divineconnectionkcs@gmail.com
- **In-App**: Settings > Privacy > Data Management

We will respond to your request within 30 days.

## 8. Children's Privacy

KCS Meet is not intended for children under the age of 13. We do not knowingly collect personal information from children under 13.

If you are a parent or guardian and believe your child has provided us with personal information, please contact us immediately at divineconnectionkcs@gmail.com. We will promptly delete such information from our systems.

**For Users Aged 13-18:**  
If you are between 13 and 18 years old, you must have parental or guardian consent to use KCS Meet.

## 9. Account and Data Deletion

### 9.1 How to Delete Your Account

You can delete your account through:

1. **In-App Method**:
   - Go to Settings > Account > Delete Account
   - Confirm deletion request
   - You will receive email confirmation

2. **Email Request**:
   - Send email to divineconnectionkcs@gmail.com
   - Subject: "Account Deletion Request"
   - Include your registered email address
   - We will process within 7 business days

3. **Web Portal**:
   - Visit: https://meet.krishnaconsciousnesssociety.com/account/delete
   - Follow the deletion process

### 9.2 What Happens When You Delete Your Account

- Your profile information will be permanently deleted within 30 days
- Your meeting history will be anonymized
- Your chat messages will be deleted (except where required by law)
- Your payment transaction history will be retained for 7 years for tax compliance
- You will no longer be able to access your account or associated data

### 9.3 Data Deletion for Specific Data Types

To request deletion of specific data (without deleting your account):
- Email: divineconnectionkcs@gmail.com
- Specify the data type you want deleted
- We will process within 14 business days

## 10. International Data Transfers

KCS Meet is operated from India. If you access our services from outside India, your information may be transferred to, stored, and processed in:

- **India**: Primary data center location
- **United States**: Cloud service providers (AWS, Vercel)
- **European Union**: CDN and backup servers

We ensure appropriate safeguards are in place for international transfers, including:
- Standard Contractual Clauses (SCCs) with service providers
- Adequacy decisions by relevant data protection authorities
- Compliance with GDPR, CCPA, and other applicable data protection laws

## 11. Cookies and Tracking Technologies

### 11.1 Types of Cookies We Use

- **Essential Cookies**: Required for authentication and core functionality
- **Analytics Cookies**: Help us understand how you use the platform
- **Preference Cookies**: Remember your settings and preferences
- **Session Cookies**: Maintain your login session

### 11.2 Third-Party Cookies

We use the following third-party analytics:
- **Vercel Analytics**: Website performance and usage metrics
- **Firebase Analytics**: App usage and crash reporting

### 11.3 Your Cookie Choices

You can control cookies through your browser settings. Note that disabling cookies may affect functionality.

## 12. California Privacy Rights (CCPA)

If you are a California resident, you have additional rights:

- **Right to Know**: What personal information we collect, use, disclose, and sell
- **Right to Delete**: Request deletion of your personal information
- **Right to Opt-Out**: We do not sell personal information
- **Right to Non-Discrimination**: We will not discriminate against you for exercising your rights

**To exercise your CCPA rights:**  
Email: divineconnectionkcs@gmail.com  
Subject: "CCPA Privacy Rights Request"

## 13. European Privacy Rights (GDPR)

If you are in the European Economic Area (EEA), you have rights under GDPR:

**Legal Basis for Processing:**
- **Consent**: For optional features like marketing communications
- **Contract Performance**: To provide meeting services
- **Legitimate Interests**: For analytics, security, and improvements
- **Legal Obligations**: For compliance with laws

**Data Protection Officer:**  
Email: dpo@krishnaconsciousnesssociety.com

**Supervisory Authority:**  
You have the right to lodge a complaint with your local data protection authority.

## 14. Do Not Track Signals

We currently do not respond to "Do Not Track" (DNT) signals from web browsers.

## 15. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of any changes by:

- Posting the new Privacy Policy on this page
- Updating the "Last Updated" date
- Sending an email notification (for significant changes)
- In-app notification

**Your continued use of KCS Meet after changes constitutes acceptance of the updated Privacy Policy.**

## 16. Contact Us

If you have questions, concerns, or requests regarding this Privacy Policy or our privacy practices:

**Krishna Consciousness Society**

**Email**: divineconnectionkcs@gmail.com  
**Website**: https://meet.krishnaconsciousnesssociety.com  
**Address**:  
F-408, Ghar Aangan Flats  
Muhana Mandi, 302029  
Jaipur, Rajasthan, India

**Response Time**: We aim to respond to all privacy inquiries within 7 business days.

---

## 17. Definitions

- **Personal Data**: Information that identifies or can identify an individual
- **Processing**: Any operation performed on personal data
- **Data Controller**: Entity that determines purposes and means of processing
- **Data Processor**: Entity that processes data on behalf of controller
- **User**: Any person who uses KCS Meet services

---

**By using KCS Meet, you acknowledge that you have read and understood this Privacy Policy.**
