...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to save meeting metadata' }, { status: 500 });
    }

    return NextResponse.json({ message: 'Meeting metadata saved successfully', data }, { status: 200 });
  } catch (e) {
